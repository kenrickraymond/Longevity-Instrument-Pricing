...
nsim = 500
iter = 100
updated_forecast = matrix(nrow=iter,ncol=h)
for(i in 1:iter){
  ...
  updated_forecast[i,] = rowQuantiles(simulate(updated_fit, h=h)$rates["65",,], probs=0.5)
}

updated_forecast[,1] = rowQuantiles(simulate(updated_fit, h=h)$rates["65",,], probs=0.5)

matlines(LCsim$years, colMeans(updated_forecast)) # CENTRAL VAR, UNSTRESSED
